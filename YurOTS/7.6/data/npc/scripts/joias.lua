...
selfSay('Olá ' .. creatureGetName(cid) .. '! Eu sou fanático por jóias e as compro! ')
focus = cid
talk_start = os.clock()
elseif string.find(msg, '(%a*)oi(%a*)') and (focus ~= cid) and getDistanceToCreature(cid) < 3 then
selfSay('Desculpe, ' .. creatureGetName(cid) .. '! Eu falo com você em um minuto.') elseif focus == cid then talk_start = os.clock() 
...
elseif msgcontains(msg, 'oferta') then
selfSay('Veja os quadros com as ofertas.')

elseif string.find(msg, '(%a*)tchau(%a*)') and getDistanceToCreature(cid) < 3 then
