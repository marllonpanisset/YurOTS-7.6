 ...
 function onCreatureSay(cid, type, msg)
 	...
 	if ((string.find(msg, '(%a*)hi(%a*)')) and (focus == 0)) and getDistanceToCreature(cid) < 3 then
 		selfSay('Eae ' .. creatureGetName(cid) .. '! Eu vendo todos os equips: Shields, Helmets, Legs, Armors, Weapons, Amulets and Rings. Diga o nome do item, que te falo o valor para vc comprar!')
 		focus = cid
 		talk_start = os.clock()
 	end
 	...
 	if string.find(msg, '(%a*)bye(%a*)') and focus == cid and getDistanceToCreature(cid) < 3 then
 		selfSay('Até mais, e aproveite o item...')
 		focus = 0
 		talk_start = 0
 	end
 end
